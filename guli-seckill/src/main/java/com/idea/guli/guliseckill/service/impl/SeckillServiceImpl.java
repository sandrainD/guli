package com.idea.guli.guliseckill.service.impl;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.TypeReference;
import com.idea.common.vo.MemberRespVo;
import com.idea.guli.guliseckill.feign.CouponFeignService;
import com.idea.guli.guliseckill.feign.ProductFeignService;
import com.idea.guli.guliseckill.interceptor.LoginUserInterceptor;
import com.idea.guli.guliseckill.service.SeckillService;
import com.idea.guli.guliseckill.to.SeckillSkuRedisTo;
import com.idea.guli.guliseckill.vo.SeckillSessionsWithSkus;
import com.idea.guli.guliseckill.vo.SkuInfoVo;
...
public class SeckillServiceImpl implements SeckillService {
    ...
    public void upSeckillSkuLatest3Days() {
        ...
        if (session.getCode() == 0) {
            //上架商品
            List<SeckillSessionsWithSkus> sessionData = (List<SeckillSessionsWithSkus>) session.getData(new TypeReference<List<SeckillSessionsWithSkus>>() {
            });
            //缓存到redis
            //1.缓存活动信息
                ...
                session.getRelationSkus().stream().forEach(seckillSkuVo -> {
                    if (!ops.hasKey(seckillSkuVo.getPromotionSessionId().toString() + "_" + seckillSkuVo.getSkuId().toString())) {
                        ...
                        if (skuInfo.getCode() == 0) {
                            SkuInfoVo info = (SkuInfoVo) skuInfo.getData2("skuInfo", new TypeReference<SkuInfoVo>() {
                            });
...
//                       redis.setRandomCode(null);
    ...
    public String kill(String killId, String key, Integer num) {
        MemberRespVo respVo = (MemberRespVo)LoginUserInterceptor.loginUser.get();
        //1.获取当前秒杀商品详细信息
